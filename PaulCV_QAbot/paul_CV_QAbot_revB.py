...
# Configure logging for all LangChain components
...
def document_loader(rag_doc_dir):
    ...
    logging.info(f"Loaded {len(loaded_pages)} documents.")
    return loaded_pages
...
def text_splitter(data):
    ...
    logging.info(f"Splitted data - no of chunks: {len(flatten_chunks)}")
    logging.info(f"Splitted data - type: {type(flatten_chunks)}")
    logging.info(f"Splitted data element - type: {type(flatten_chunks[0])}")
...
def vector_database(chunks):
    ...
    logging.info(f"Number of added data chunks to Chroma vector DB: {len(ids)}")
    return vector_db
...
def retriever_qa_chain(rag_doc_dir, query, openai_llm):
    ...
    logging.info(f"Response from Q&A chain: {response['answer']}")
